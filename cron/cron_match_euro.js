...
var arr_url = {
	"http://www.soccerhighlightstoday.com/?s=euro+2016":"424"
};
	...
	download(key + (page === "1" ? "" : "/page/"+page), function(data) {
	  if (data) {
		...
		$("div.td-module-thumb > a").each(function(i, e) {
			...
			var title_temp = title.replace(/(Euro 2016)/gi, 					
			...
			title = title.replace(/(Euro 2016)/gi, "").replace(" \u0013 ", " - ");
			...
			download(href, function(detail) {
			  if (data) {
				...
				if( link_video /*&& thumbnail.indexOf("100x70") > -1 */){
						var match = {};
						match.thumbnail = thumbnail.replace("150x150","218x150").replace("100x70","218x150");
						match.title = title;
