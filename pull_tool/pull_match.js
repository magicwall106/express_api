...
var arr_url = {
	"http://www.soccerhighlightstoday.com/http:/www.soccerhighlightstoday.com/topics/uncategorized/europe/euro-2016":"424",
...
exports.pullMatch = function(req, res) {
		...
		download(key + (page === "1" ? "" : "/page/"+page), function(data) {
		  if (data) {
			...
			$("div.td-module-thumb > a").each(function(i, e) {
				...
				title = $(e).attr("title").replace("VIDEO ","")
							.replace(" Highlights","")
							.replace("(","").replace(")","");
				if(title){
					title = utf8.decode(title);
					
					title_temp = title.replace(/(Champions League)|(La Liga)|(Serie A)|(Premier League)|(Ligue 1)|(Bundesliga)|(Euro 2016)/gi, 					
					function myFunction(x){
						return arr_league[x];
					});
					
					title = title.replace(/(Champions League)|(La Liga)|(Serie A)|(Premier League)|(Ligue 1)|(Bundesliga)|(Euro 2016)/gi, "").replace(" \u0013 ", " - ");
				}
				...
				if (title_temp && title_temp.split(special_dash).length > 1) {
					...
					if(arr_team1_score1 && arr_team1_score1.length > 1){
					...
					if(arr_team2_score2 && arr_team2_score2.length > 1){
